...
- **Workflow action catalog**: `change_priority`, `add_tag`, `remove_tag`, `change_status`, `set_department`, `assign_agent`, `add_note`, `insert_canned_reply` (`{{field}}` interpolation against the ticket), `delay` (pauses the run + persists remaining actions as a `DeferredWorkflowJob` row; a per-minute poller resumes due jobs — see the [Workflows docs page](https://docs.escalated.dev/workflows) for semantics).
